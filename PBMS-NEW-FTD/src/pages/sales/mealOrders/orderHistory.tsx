...
import useOrderHistory from '../../../hooks/sales/useOrderHistory';
...
const OrderHistory = () => {
 const {data: orderHistory} = useOrderHistory()
  ...
  const handleViewOrder = (order: Bill) => {
    setSelectedOrder(order);
    setShowViewModal(true);
  };

  // Handle print receipt
  const handlePrintReceipt = (order: Bill) => {
    ...
    w.document.write(`
      ...
      <body>
        <div class="header">
          ...
          <p>Table ${order.table?.number || order.tableId}</p>
          <p>Order #${order.id}</p>
        ...
        <div style="display: flex; justify-content: space-between;">
          ...
          <span>UGX ${(totalAmount - balanceAmount).toLocaleString()}</span>
        </div>
        <div style="display: flex; justify-content: space-between;">
          <span>Balance</span>
        ...
        <div style="display: flex; justify-content: space-between;">
          ...
          <span>${order.paymentMethod || 'Multiple'}</span>
        </div>
        <div class="footer">
  ...
  const tableData = orderHistory.map(order => {
    const totalAmount = typeof order.total === 'string' ? parseFloat(order.total) : order.total;
